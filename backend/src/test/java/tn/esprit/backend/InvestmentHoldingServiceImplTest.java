...
class InvestmentHoldingServiceImplTest {
    ...
    @Test
    void shouldCreateHoldingWhenDealIsInNegotiation() {
        CreateInvestmentHoldingRequest payload = new CreateInvestmentHoldingRequest();
        ...
        when(stripePaymentService.createPaymentIntent(anyLong(), anyString(), anyString(), anyString(), anyString()))
                .thenReturn(new StripePaymentIntentData("pi_test_1", "cs_test_1"));
        ...
        var response = service.createHolding("req-1", payload, new RequestActor("investor-1", UserRole.INVESTOR));

        assertEquals("holding-1", response.getId());
        assertEquals(InvestmentHoldingStatus.WAITING_PAYMENT, response.getStatus());
        assertEquals("pi_test_1", response.getStripePaymentIntentId());
        assertEquals("cs_test_1", response.getStripeClientSecret());
    }

    @Test
    void shouldRefuseHoldingCreationOutsideNegotiation() {
        CreateInvestmentHoldingRequest payload = new CreateInvestmentHoldingRequest();
        payload.setAmount(10000L);
        payload.setCurrency("usd");
        ...
        IllegalArgumentException ex = assertThrows(
                ...
                () -> service.createHolding("req-1", payload, new RequestActor("investor-1", UserRole.INVESTOR))
        );

        assertTrue(ex.getMessage().contains("NEGOTIATION"));
    }
